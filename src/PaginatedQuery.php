...
class PaginatedQuery{
    ...
    public function getItems(string $classMapping)
    {
        ...
        if($this->items === null){
            ...
            //dd($currentPage, $pages);
            
            if($currentPage > $pages){
                throw new \Exception('Cette page n\'existe pas !');
            }

            $offset = $this->perPage * ($currentPage - 1); // Représente le décallage du nb d'items entre chaques pages ()
...
HTML;
    ...
    public function nextLink(string $link): ?string
    {
        ...
        // On retourne le lien (syntaxe heredoc pour ne pas avoir à échapper le html)
        return <<<HTML
            <a href="{$link}" class="btn btn-dark ml-auto">Page suivante &raquo;</a>
HTML;
    }
    ...
    {
        // Si la propriété "count" (de notre classe) vaut null (c'est qu'elle n'est pas défini) alors... (OPTIMISATION des performances)
        if($this->count === null){
